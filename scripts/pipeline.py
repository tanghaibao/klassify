...
def run_pipe(cmd1, cmd2, *, cwd=None, stdout_path: Path | None = None):
    logging.info("PIPE: %s | %s", " ".join(map(str, cmd1)), " ".join(map(str, cmd2)))
...
def main():
    ...
    ap.add_argument("--workdir", type=Path, default=Path("."), help="Working directory (default: .)")
    ap.add_argument("--scripts-dir", type=Path, default=DEFAULT_SCRIPT_PATH,
                    help="Directory containing helper scripts (split_reads_by_breakpoint.py, cluster_to_paired_regions.py).")
    ap.add_argument("--kmers", type=Path, default=None, help="Prebuilt kmers.bc (skip build if provided).")
    ap.add_argument("--threads-minimap2", type=int, default=32, help="Threads for minimap2 (-t)")
    ap.add_argument("--threads-sort", type=int, default=8, help="Threads for samtools sort (-@)")
    ap.add_argument("--preset", default="map-hifi", choices=["map-hifi", "map-ont"],
                    help="Minimap2 preset (default: map-hifi)")
    ap.add_argument("--force", action="store_true", help="Overwrite/redo steps even if outputs exist")
    ...
    split_script = (scripts_dir / "split_reads_by_breakpoint.py") if scripts_dir else Path(
        "split_reads_by_breakpoint.py")
    cluster_script = (scripts_dir / "cluster_to_paired_regions.py") if scripts_dir else Path(
        "cluster_to_paired_regions.py")

    if not split_script.exists():
        logging.warning("split_reads_by_breakpoint.py not found at %s (will try anyway if it's in PATH).", split_script)
    if not cluster_script.exists():
        logging.warning("cluster_to_paired_regions.py not found at %s (will try anyway if it's in PATH).",
                        cluster_script)

    # 1) Build unique k-mers from parental genomes
    if args.kmers and kmers_bc.exists():
        logging.info("Using prebuilt k-mer DB: %s", kmers_bc)
    elif not need_update(args.parents_ref, kmers_bc) and not args.force:
        logging.info("Found kmers DB, skipping build: %s", kmers_bc)
    else:
    ...
    if not need_update([kmers_bc, args.f1_reads], f1_filtered_tsv) and not args.force:
        logging.info("F1 classify already done: %s", f1_filtered_tsv)
    else:
        run(["klassify", "classify", str(kmers_bc), str(args.f1_reads), "-o", "f1_classify"], cwd=work)

    if not need_update([f1_filtered_tsv, args.f1_reads], f1_extracted_fa) and not args.force:
        logging.info("F1 extract already done: %s", f1_extracted_fa)
    else:
        run(["klassify", "extract", str(f1_filtered_tsv), str(args.f1_reads), "-o", str(f1_extracted_fa)], cwd=work)

    if not need_update([f1_extracted_fa, args.parents_ref], f1_bam) and not args.force:
        logging.info("F1 alignment already exists: %s", f1_bam)
    else:
        run_pipe(
            ["minimap2", "-t", str(args.threads_minimap2), "-ax", args.preset, "--eqx", "--secondary=no",
             str(args.parents_ref), str(f1_extracted_fa)],
            ["samtools", "sort", "-@",
             str(args.threads_sort), "-o", str(f1_bam)],
            cwd=work, stdout_path=f1_bam
        )

    # 3) Classify parent reads (control), extract, map
    if not need_update([kmers_bc, args.parent_reads], parent_filtered_tsv) and not args.force:
        logging.info("Parent classify already done: %s", parent_filtered_tsv)
    else:
        run(["klassify", "classify", str(kmers_bc), str(args.parent_reads), "-o", "parent_classify"], cwd=work)

    if not need_update([parent_filtered_tsv, args.parent_reads], parent_extracted_fa) and not args.force:
        logging.info("Parent extract already done: %s", parent_extracted_fa)
    else:
        run(["klassify", "extract", str(parent_filtered_tsv), str(args.parent_reads), "-o", str(parent_extracted_fa)],
            cwd=work)

    if not need_update([parent_extracted_fa, args.parents_ref], parent_bam) and not args.force:
        logging.info("Parent alignment already exists: %s", parent_bam)
    else:
        run_pipe(
            ["minimap2", "-t", str(args.threads_minimap2), "-ax", args.preset, "--eqx", "--secondary=no",
             str(args.parents_ref), str(parent_extracted_fa)],
            ["samtools", "sort", "-@",
             str(args.threads_sort), "-o", str(parent_bam)],
            cwd=work, stdout_path=parent_bam
        )

    # 4) Regions present in F1 but NOT parent (control)
    if not need_update([f1_bam, parent_bam], regions_tsv) and not args.force:
        logging.info("Regions TSV already exists: %s", regions_tsv)
    else:
    ...
    if not need_update([regions_tsv, f1_bam], regions_fa) and not args.force:
        logging.info("Regions FASTA already exists: %s", regions_fa)
    else:
    ...
    if not need_update(split_input, split_output) and not args.force:
        logging.info("Split reads already exist: %s", split_output)
    else:
    ...
    if not need_update(split_output, roi_bam) and not args.force:
        logging.info("ROI BAM already exists: %s", roi_bam)
    else:
        run_pipe(
            ["minimap2", "-t", str(args.threads_minimap2), "-ax", args.preset, "--eqx", "--secondary=no",
             str(args.parents_ref), str(split_output)],
            ["samtools", "sort", "-@",
             str(args.threads_sort), "-o", str(roi_bam)],
            cwd=work, stdout_path=roi_bam
        )

    # BAM → BED
    if not need_update(roi_bam, roi_bed) and not args.force:
        logging.info("ROI BED already exists: %s", roi_bed)
    else:
    ...
    if not need_update(roi_bed, paired_regions) and not args.force:
        logging.info("Paired regions TSV already exists: %s", paired_regions)
    else:
        with open(roi_tsv, "wb") as out:
            run([sys.executable, str(cluster_script), str(roi_bed)], cwd=work, stdout=out)

    logging.info("Done.")
    logging.info("Key outputs:\n"
                 f"  k-mers:          {kmers_bc}\n"
                 f"  F1 BAM:          {f1_bam}\n"
                 f"  Parent BAM:      {parent_bam}\n"
                 f"  Regions TSV:     {regions_tsv}\n"
                 f"  Regions FASTA:   {regions_fa}\n"
                 f"  ROI BAM:         {roi_bam}\n"
                 f"  ROI BED:         {roi_bed}\n"
                 f"  ROI TSV:         {roi_tsv}\n"
                 f"  Paired Regions:  {paired_regions}\n")
